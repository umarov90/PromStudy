...
public class TestPromoter2 {
    ...
    public static void main(String[] args) {
        ...
        if (args.length > 0) {
            try {
                ...
                String[] sets = new String[]{"TATA+.resp", "TATA-.resp", "TATA-+.resp"};
                double[][] res = new double[3][];
                for (int s = 0; s < 3; s++) {
                    String set = sets[s];
                    res[s] = evaluate(new File(set));
                }
                System.out.print("Promoter2");
                System.out.print(", ");
    ...
    private static double[] evaluate(File setp) throws FileNotFoundException {
        ...
        while(scan.hasNextLine()){
            ...
            if(line.startsWith("  Position  Score  Likelihood")){
                s++;
            }
        ...
        try (PrintWriter out = new PrintWriter("Promoter2.val")) {

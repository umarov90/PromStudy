    ...
    public static void main(String[] args) {
        ...
        if (args.length > 0) {
            try {
                ...
                String[] sets = new String[]{"TATA+.resw", "TATA-.resw", "TATA-+.resw"};
                double[][] res = new double[3][];
                for (int s = 0; s < 3; s++) {
                    String set = sets[s];
                    res[s] = evaluate(new File(set));
                }
                System.out.print("TSSW");
                System.out.print(", ");
    ...
    private static double[] evaluate(File setp) throws FileNotFoundException {
       ...
       StringBuilder sb = new StringBuilder();
        while (scan.hasNextLine()) {
            ...
            if (line.startsWith(" Promoter Pos:") || line.startsWith(" Enhancer Pos:")) {
        ...
        try (PrintWriter out = new PrintWriter("TSSW.val")) {
